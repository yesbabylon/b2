...
# Do nothing if the address is already present
if grep -Fq "$ADDRESS" "$NETPLAN_FILE"; then
    echo "Address $ADDRESS is already present."
    exit 0
fi
...
echo "Address $ADDRESS added to veth0."
