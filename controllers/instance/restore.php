...
function instance_restore(array $data): array {
    ...
    exec("docker exec $db_hostname /usr/bin/mysql -u $db_backup_username --password=$db_backup_password -e \"DROP DATABASE IF EXISTS equal; CREATE DATABASE equal;\"", $output);
    file_put_contents('/root/b2/logs/tmp.log', json_encode($output));
